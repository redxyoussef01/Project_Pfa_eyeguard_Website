...
function Home() {
    ...
    const fetchPersonnels = async () => {
        try {
            ...
            if (response.data && Array.isArray(response.data.autorisations)) {
                setPersonnels(response.data.autorisations);
            } else {
                setPersonnels([]);
            }
        } catch (error) {
    ...
    const fetchPersonnes = async () => {
        try {
            ...
            if (response.data && Array.isArray(response.data.personnes)) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post('http://localhost:8000/api/personnels/new', formDataToSend);
            setShow(false);
            fetchPersonnels(); // Refresh personnels data after adding a new personnel
        } catch (error) {
            console.error('Error adding new personnel:', error);
            alert('Error adding new personnel');
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:8000/api/personnels/delete/${id}/`);
                    ...
                    <div className="col-sm-3 mt-5 mb-4 text-gred">
                        <div className="search">
                            <form className="form-inline">
                                <input className="form-control mr-sm-2" type="search" placeholder="Search personnel" aria-label="Search" />
                            </form>
                        </div>
                    </div>
                    <div className="col-sm-3 offset-sm-2 mt-5 mb-4 text-gred" style={{ color: "green" }}><h2><b>personnels List</b></h2></div>
                        ...
                        <table className="table table-striped table-hover table-bordered">
                            ...
                            <tbody>
                                {personnels.map((personnel, index) => (
                                    <tr key={index}>
                                        <td>{index + 1}</td>
                                        <td>{personnel.nom}ssssss</td>
                                        <td>{personnel.prenom}</td>
                                        <td>{personnel.email}</td>
                                        <td>
                                            <a href="#" className="edit" title="Edit" data-toggle="tooltip" onClick={() => handleEdit(personnel)}>
                                                <i className="material-icons">&#xE254;</i>
                                            </a>
                                            <a href="#" className="delete" title="Delete" data-toggle="tooltip" onClick={() => handleDelete(personnel.id)}>
                                                <i className="material-icons">&#xE872;</i>
                                            </a>
                                        </td>
                                    </tr>
